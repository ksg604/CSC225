...
public class PairProduct{
	...
	*/
	public static boolean PairProduct225(int[] A){
		...
		//Create an array T which will keep track of values between 1 and 
		//225 that are stored in A.  
		
		//T[j] will be the number of times that j appears.
		//Every index in T will have a value of 0 initially.
		for (int i = 0; i < n; i++) {
		...
		//Iterate through A and use the array T to ensure that
		//variables x and y are factors of 225.
		for (int j = 0; j < n; j++) {
			...
			if (x * y == 225 && T[x] > 0 && T[y] > 0) {
				System.out.printf("Pair: (%d,%d)",x , y);
				return true;
			}
		}
		
		//Original Solution running time O(nlogn)
	/*	for(int i = 0; i < n; i++) {
			for(int j = i+1; j < n; j++) {
				if (A[i] * A[j] == 225) {
					int x = A[i];
					int y = A[j];
					System.out.printf("Output Pair: %d, %d\n", x, y);
					return true;
				}
			}
		}*/
