...
public class HashTable{
	...
	*/
	...
	private int getMaxThreshold(float tableSize) {
		
		//max threshold
		tableSize = T.length;
		return (int)(0.75*tableSize);
	}
	
			
	private int getMinThreshold(float tableSize) {
		
		//min threshold
		tableSize = T.length;
		return (int)(0.25*tableSize);
	}
...
public int insert(String s){
		int i = hash(s);
		int j = 0;
		double loadFactor = (NumElements+1)/ TableSize;
		
		while(loadFactor > 0.75) {
			resizeHandler(2*TableSize);
			loadFactor = (NumElements+1) / TableSize;
		}
